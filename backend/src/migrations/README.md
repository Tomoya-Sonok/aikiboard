...
3. `001_*.sql` の中身を貼り付け **Run**。エラーが無ければ次のファイルへ。`014` まで実行する
...
`cd backend && pnpm exec supabase db reset` で `000`〜`014` を番号順に再適用する。
...
- 適用後、Studio(`http://127.0.0.1:54323`)で `aikiboard` schema(`membership_requests` 含む)を確認できる
