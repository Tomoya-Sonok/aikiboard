...
2. 入力テキストを読み、内容をテンプレートの各節に振り分ける。
   ...
   - **入力にある課題記述(「誰の/何を/なぜ」等)をユーザーストーリー形式へ言い換えるのは可**。ただし入力に無い主語・目的・理由を足さないこと。足したくなったらその部分は `[TBD]`。
     **Why**: 前回、要望メモの課題記述をユーザーストーリー形式に整形したが、「整形」と「創作」の線引きがスキルに書かれておらず、実行時の判断に委ねられていた。
