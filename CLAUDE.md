...
> **ステータス: Phase 1(MVP)の主要機能が一巡(2026-06-20、PR #51〜#99 マージ済み)**
>
> リポジトリ初期化と基盤構築(Phase 0、2026-05-05 完了)に続き、Phase 1 で **認証・ボード作成・ダッシュボード/シェル・稽古カレンダー + 出欠・お知らせ配信・メンバー管理・フィード + スレッド + AikiNote 連携・通知(アプリ内)・アクティビティログ + feature_flag 基盤・道場ページ(公開)・道場マスタ双方向書き込み・アーカイブ・会計・ボード Todo** を実装。フロントエンドは Vercel(`https://aiki-board.com`)、バックエンドは Cloudflare Workers(`https://api.aiki-board.com`)で稼働、Supabase の `aikiboard` スキーマ + RLS 適用済み(REST 公開 + service_role 権限付与済み)。開発は **PR ベース運用**(main 直 push 禁止)、運用方針は `docs/adr/`。**残タスクと実装方針は普及ロードマップ [`docs/roadmap.md`](docs/roadmap.md) が最新**。
>
> - 普及ロードマップ(残タスク台帳): [`docs/roadmap.md`](docs/roadmap.md)
