...
## spec-kit スキルは使わない(2026-09-10 決定)

- **`/speckit-*` スキルは使わず、`spec.md` / `plan.md` / `tasks.md` は手書きする**。`.claude/skills/speckit-*` と `.specify/` は削除せず残すが、ワークフローには組み込まない。
  **Why**: spec-kit は `specs/NNN-<name>/` の連番ディレクトリ + `.specify/feature.json` を前提とするが、このリポジトリは `specs/{feature}/`(機能名がディレクトリ名)で14機能分の資産があり、噛み合わなかった。PR #115 の実行では `/speckit-specify` の後、実際には plan/tasks のスキルが起動できず手書きになった。
- 判断の理由(記録として): ①「既存機能の変更か新機能か」で運用を分ける案は、境界が曖昧で毎回判断が必要になる(#115 は既存機能への機能追加だった)②`specs/members/spec.md` という機能名ディレクトリの参照性を、連番移行で失うのは割に合わない ③spec-kit から実際に得ていた価値は specify のテンプレート思想のみで、plan/tasks は手書きの方がプロジェクト固有の事情を反映できた。
- **報告時のルール**: スキルを起動していないのに「スキルを実行した」と書かない。手書きしたなら手書きと書く。
  **Why**: PR #115 で「`/speckit-plan` → `/speckit-tasks` を実行します」と宣言しながら、実際には起動せず手で作成した。宣言と実態がずれた。
